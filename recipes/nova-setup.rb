...
if node["nova"]["networks"]["public"]["bridge_dev"] == node["nova"]["network"]["public_interface"]
  node.set["nova"]["network"]["public_interface"] = node["nova"]["networks"]["public"]["bridge"]
end
...
node["nova"]["networks"].each do |net, v|
  nova_network_networks "Creating #{v['label']}" do
    ...
    dns1 v['dns1']
    dns2 v['dns2']
    Chef::Log.debug "Vlan ID setting now?"
